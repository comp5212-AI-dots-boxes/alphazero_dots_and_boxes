...
class MCTS(object):
    ...
    def get_move_probs(self, state: GameBase, temp=1e-3):
...
class MCTSPlayer(Player):
    ...
    #     def get_action(self, game: GameBase, temp=1e-3, return_prob=0):
    #         available_actions = game.get_available_actions()
    #         # the pi vector returned by MCTS as in the alphaGo Zero paper
    #         move_probs = np.zeros((5, 5, 2))
    #         if len(available_actions) > 0:
    #             acts, probs = self.mcts.get_move_probs(game, temp)
    #             for idx in range(len(acts)):
    #                 move_probs[acts[idx]] = probs[idx]
    #             move_probs = move_probs.reshape((50,))
    #             if self._is_selfplay:
    #                 # add Dirichlet Noise for exploration (needed for
    #                 # self-play training)
    #                 move = np.random.choice(
    #                     [idx for idx in range(len(acts))],
    ...
    #                 move = np.random.choice([idx for idx in range(len(acts))], p=probs)
    #                 move = acts[move]
    #                 # reset the root node
    #                 self.mcts.update_with_move(-1)
