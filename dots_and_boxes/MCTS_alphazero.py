...
from dots_and_boxes.dots_and_boxes import DotsAndBoxes, DotsAndBoxesPlayerBase
...
class TreeNode(object):
    ...
    def expand(self, action_priors, game: DotsAndBoxes):
        ...
        for action, prob in action_priors:
            if action not in self._children:
                tmp = game.copy()
                tmp.act(action)
                next_player = tmp.current_player_id
    ...
    def update_recursive(self, leaf_value):
        ...
        # the score of root node is meaningless
        if self._parent is None:
            return
        # self._parent.player is the player who do this action and this player
        if self._parent.player != self.player:
...
class MCTS(object):
    ...
    def __init__(self, policy_value_fn, player, c_puct=5, n_playout=10000):
        """
    ...
    def _playout(self, state: DotsAndBoxes):
    ...
    def get_move_probs(self, state: DotsAndBoxes, temp=1e-3):
    ...
    def update_with_move(self, last_move, player):
...
class MCTSPlayer(DotsAndBoxesPlayerBase):
    ...
    def set_player_ind(self, p):
        self.player = p
        self.mcts.update_with_move(-1, p)
    ...
    def get_action(self, game: DotsAndBoxes, temp=1e-3, return_prob=0):
        ...
        if len(sensible_moves) > 0:
            ...
            if self._is_selfplay:
                ...
                self.mcts.update_with_move(move, next_player)
            else:
                # with the default temp=1e-3, it is almost equivalent
                # to choosing the move with the highest prob
                move = np.random.choice(acts, p=probs)
                # reset the root node
                self.mcts.update_with_move(-1, self.player)
